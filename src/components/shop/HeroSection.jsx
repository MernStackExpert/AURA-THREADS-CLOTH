...
export default function HeroSection({ mainBanners = [], sideBanners = [] }) {
  ...
  return (
    <section className="w-full bg-background pt-0 md:pt-6 pb-8">
      <div className="w-full max-w-[1450px] mx-auto md:px-8">
        <div className="w-full h-[55vh] min-h-[400px] md:h-[80vh] md:min-h-[600px] flex flex-col md:flex-row overflow-hidden bg-foreground/5">
          <div className="relative w-full md:w-2/3 h-full overflow-hidden group">
              ...
              {activeMain && (
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-14">
                    ...
                    >
                      {activeMain.title && (
                        <h1 className="text-3xl md:text-5xl lg:text-6xl font-light text-white uppercase tracking-[0.15em] leading-snug md:leading-tight mb-4 md:mb-6">
                          {activeMain.title}
                        </h1>
                      )}
                      {activeMain.link && (
            ...
            {mainBanners.length > 1 && (
              <div className="absolute bottom-6 left-6 md:bottom-10 md:left-14 flex gap-4 md:gap-6 z-20">
                {mainBanners.map((_, idx) => (
                  <button
                    key={idx}
                    onClick={() => setCurrentMainIndex(idx)}
                    className="group flex items-center gap-2 md:gap-3 py-2 cursor-pointer"
                  >
                    <span
                      className={`text-[9px] md:text-[10px] font-medium tracking-[0.2em] transition-colors duration-500 ${
                    ...
                    <div
                      className={`h-[1px] transition-all duration-700 ease-[0.22,1,0.36,1] ${
                        currentMainIndex === idx
                          ? "w-10 md:w-16 bg-white"
                          : "w-4 md:w-6 bg-white/30 group-hover:bg-white/60"
                      }`}
                    />
                  </button>
                ))}
          ...
          <div className="hidden md:block relative w-1/3 h-full overflow-hidden border-l border-background group">
            <AnimatePresence mode="sync">
              {activeSide && (
                <motion.div
                  key={activeSide._id}
