...
export default function HeroSection({ mainBanners = [], sideBanners = [] }) {
  ...
  const activeMainBanner = mainBanners[currentIndex];
    ...
    <section className="w-full h-[85vh] md:h-[80vh] min-h-[600px] flex flex-col md:flex-row bg-background">
          ...
          {activeMainBanner && (
            ...
            >
              <Image
                ...
                alt={activeMainBanner.title || "Premium Collection"}
                fill
      ...
      {activeSideBanner && (
        <div className="hidden md:block relative w-1/3 h-full overflow-hidden border-l border-border/20 group">
          <Image
            src={activeSideBanner.image}
            alt={activeSideBanner.title || "Exclusive Offers"}
            fill
            priority
